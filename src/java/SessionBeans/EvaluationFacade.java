...
public class EvaluationFacade extends AbstractFacade<Evaluation> {
    ...
    public int findEvaluation(Places place, Users user) {
        ...
        criteriaQuery.where(criteriaBuilder.equal(joinUser.get(Users_.username), user.getSurname()));
        criteriaQuery.where(criteriaBuilder.equal(joinPlace.get(Places_.id), place.getId()));
    ...
    public Evaluation getEvaluation(Places place, Users find) {
        ...
        //Indiquer les conditions
        criteriaQuery.where(criteriaBuilder.equal(joinUser.get(Users_.username), find.getSurname()));
        criteriaQuery.where(criteriaBuilder.equal(joinPlace.get(Places_.id), place.getId()));
        //Création de la requête
        Query q = em.createQuery(criteriaQuery);
        //Retour du résultat
